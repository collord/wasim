...
| 2 ✅ | **`gather`/reindex-by-index** + **standalone `ordinal`** — `gather(x, idx)` (`out[k]=x[idx[k]-1]`, rides `idx`'s axis) and `ordinal(x)` (1-based positions along the axis, the standalone `@`). No new AST node — both are `BuiltinFn`s via `map_shape`, so the dimensioned lane inherits them. Converter now lowers `x[Dim=indexRef]` → `gather` and `@Index` → `ordinal`. *Shipped: `array_language_v2.rs` checks both on both lanes bit-identically, and **`cumulate(gather(amount, sort_index(marg)))` reproduces Analytica's `Cum_reduction` exactly** (`[8,18,23,73,88,123,143,148]`).* | M | Med |
| 3 ✅ | **`null`→NaN** value policy: nullary `null()` builtin → `f64::NaN`; the vector-cond `if` now **broadcasts a scalar branch**, so `… else null()` fills every false cell with NaN (the staircase-gap idiom) rather than only cell 0. Converter maps `null`/`Undefined` → `null()`. *Shipped: `array_language_v2::null_broadcasts_as_nan_gaps_on_both_lanes`.* | M | Low–Med |
| 4 ✅ | **Derived permutation index** (B) via **converter demotion**: a computed Analytica `Index` (e.g. `SortIndex(x)`) — which can't be a static WASiM dimension — is emitted as a computed *variable* holding the derived values, so downstream `x[Dim=thisIndex]` gathers and `@thisIndex` ordinals resolve against it. *Shipped in the converter (`_build_index_element`); `Sorted_action` now converts.* The engine dimension model stays deliberately static — a *first-class* derived index is not added. | M–L | Med |
| — | General computed indexes (C): `Concat`/`CopyIndex`/index arithmetic — still deferred (the `Cumulative_reduction` plot-origin). | L | later |
