...
| `<model>` (multiple) + `<module>` | `container_def` with `kind:"submodel"` (§12) + `interface` | transform | See "module" row; the correspondence is real but the semantics differ (deterministic vs. nested realization loop). |
...
| `<module>` / `<connect from= to=>` | `container_def kind:"submodel"` + `interface.inputs[]{input,from}` / `outputs[]` | transform | Structural match: `<connect>` edges → interface bindings. **BUT** a WaSim submodel runs a *nested realization loop* and the parent reads a **reduced statistic** (`submodel_stat`); XMILE modules are plain deterministic sub-calls. For a deterministic import, bind `n_realizations:1` so the submodel is a single deterministic sub-run — see §3. |
